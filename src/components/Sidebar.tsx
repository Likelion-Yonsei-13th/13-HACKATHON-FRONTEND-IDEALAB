...
export default function Sidebar() {
  ...
  const handleCreate = (p: NewProject) => {
    ...
    localStorage.setItem(
      `meta:${createdId}`,
      JSON.stringify({ section: label, title: p.title })
    );
    localStorage.setItem(
      "ws:breadcrumb",
      JSON.stringify({ section: label, title: p.title })
      ...
      prev.map((s) => {
        ...
        return {
          ...
          items: (s.items ?? []).map((it) =>
            it.id === editing.id
              ? { ...it, title: val.title, color: val.color ?? it.color }
              : it
          ),
        };
      })
    ...
    try {
      ...
      if (location.pathname.includes(`/ws/${editing.id}`)) {
        localStorage.setItem(
          "ws:breadcrumb",
          JSON.stringify({
            section: sectionLabel(editing.section),
            title: val.title,
          })
        );
      }
    ...
    >
              ...
              {!collapsed && (
                <>
                  ...
                  <img
                    src="/logos/Lab.png"
                    alt="Lab"
                    className="h-7 -ml-6 relative z-10"
                  />
                </>
              )}
            </button>
      ...
      <EditItemModal
        ...
        initialTitle={
          editing
            ? sections
                .find((s) => s.key === editing.section)
                ?.items?.find((i) => i.id === editing.id)?.title ?? ""
            : ""
        }
        initialColor={
          editing
            ? sections
                .find((s) => s.key === editing.section)
                ?.items?.find((i) => i.id === editing.id)?.color
            : undefined
        }
