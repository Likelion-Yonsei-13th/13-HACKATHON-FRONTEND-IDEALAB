...
/* 데모 모드: 환경변수 또는 ?demo=1 */
const DEMO_MODE =
  (typeof process !== "undefined" &&
    process.env.NEXT_PUBLIC_RIGHTTAB_DEMO === "1") ||
...
function makeDemo(selectedGu: string) {
  ...
  return {
    ...
    summaries: {
      day: {
        heading: `${selectedGu} 요일 매출 인사이트`,
        paragraph: "주말에 매출이 높게 형성됩니다.",
      },
      gender: {
        heading: "성별 매출 인사이트",
        paragraph: "여성 비중이 소폭 높습니다.",
      },
      time: {
        heading: "시간대 매출 인사이트",
        paragraph: "저녁 시간대에 매출이 집중됩니다.",
      },
      age: {
        heading: "연령대 매출 인사이트",
        paragraph: "20–30대가 주 고객층입니다.",
      },
    },
    aiRecommendations: [
      { name: "핫플 구 이동", gu: "마포구" },
...
export default function RightTabEmbed({ backendGu, className }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchWithFallback = async () => {
      ...
      if (remoteBase) {
        try {
          const url1 = withQuery(remoteBase, {
            signgu_cd: code,
            sigungu_cd: code,
            yyq,
          });
      ...
      // 2) 로컬 Next API 라우트 (있다면)
      try {
        const url2 = withQuery("/api/analytics/industry-metrics", {
          signgu_cd: code,
          sigungu_cd: code,
          yyq,
        });
        ...
        <div className="px-8 pb-8">
          <div className="flex flex-col gap-8">
            ...
            <div className="p-4 border rounded-lg">
              ...
              <PieChart data={genderData} title="성별 매출 현황" />
            </div>
            ...
            <div className="p-4 border rounded-lg">
              ...
              <LineChart data={timeData} title="시간대별 매출 현황" />
            </div>
